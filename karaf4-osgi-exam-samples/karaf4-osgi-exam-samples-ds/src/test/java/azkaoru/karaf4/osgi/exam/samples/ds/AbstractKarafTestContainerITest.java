...
public abstract class AbstractKarafTestContainerITest {
	...
	public Option[] config() {
		...
		return new Option[] { karafDistributionConfiguration().frameworkUrl(KARAF_URL.version(karafVersion))
				...
				features(maven().groupId("azkaoru-karaf4-osgi-exam").artifactId("karaf4-osgi-exam-samples-ds")
						.version("1.0.0").classifier("features").type("xml"), "osgi-exam-ds") };
	}
	...
	public void testCalc() throws Exception {
		...
		Bundle bundle = findBundle("azkaoru-karaf4-osgi-exam.karaf4-osgi-exam-samples-ds");

		assertNotNull(bundle);

		ServiceReference<Calculator> sr = bc.getServiceReference(Calculator.class);
		Calculator calculator = bc.getService(sr);
